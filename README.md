...
- [twzrd-agent-intel](https://intel.twzrd.xyz) - Solana-native x402 MCP server for AI agent trust scoring in Claude Code workflows. 4 free preflight tools score any Solana wallet; `get_trust_receipt` returns a signed `twzrd.receipt.v5` via HTTP 402 + USDC. Zero-install: `claude mcp add twzrd-agent-intel --transport http https://intel.twzrd.xyz/mcp`. MCP Registry: `xyz.twzrd.intel/twzrd-agent-intel`. *By [@twzrd-sol](https://github.com/twzrd-sol)*
...
### Security & Systems
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - Trust scoring MCP for AI agents on Solana. Verify wallet reputation before x402 micropayments. Free: `score_agent(wallet)`, `preflight_check(wallet)`. Paid: `get_trust_receipt(wallet)` (x402). Config: `{"mcpServers":{"twzrd-agent-intel":{"url":"https://intel.twzrd.xyz/mcp"}}}`
